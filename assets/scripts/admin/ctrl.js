(function() {
    ...
    App.controller("PPPOEServerCtrl", function($scope, Translator, PPPOEService, Interfaces, $uibModal, toastr, $filter, Upload, $ngConfirm) {
    		...
    		$scope.toggleSort = function(sort){
    			if(!(sort in $scope.sort)){
    				$scope.sort = {} //reset
    			}
    			const v = $scope.sort[sort]
    			$scope.sort[sort] = v === 'asc' ? 'desc' : v === 'desc' ? null : 'asc'
    			$scope.loadClients()
    		}
        $scope.loadClients = function() {
            ...
            return PPPOEService.clients($scope.sort).then(function(resp) {
            ...
            $uibModal.open({
                ...
                controller: function($scope, PPPOEService, $uibModalInstance, CatchHttpError) {
                    ...
                    $scope.computeExpMinutes = function() {
                        $scope.expire_minutes = !$scope.no_expiration ? $scope.opts.expiration_unit == "hours" ? $scope.opts.expire_hours * 60 : $scope.opts.expire_hours * 60 * 24 : null
                    };
                    ...
                    $scope.updateClient = function() {
                        ...
                        if ($scope.extend_expiration) {
                            ...
                            client.expiration_date = new Date(exp_date.getTime() + $scope.expire_minutes * 6e4)
                        }
        ...
        $scope.downloadBackup = function() {
            ...
            $scope.clients.forEach(function(c, i) {
                ...
                txt += c.max_upload + ",";
                txt += (c.started_at || "") + ",";
                txt += $filter("expirationtext")({
                    expire_minutes: c.expire_minutes,
